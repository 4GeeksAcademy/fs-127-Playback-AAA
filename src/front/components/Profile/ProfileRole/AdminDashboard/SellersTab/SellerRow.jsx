...
const STATUS_CONFIG = {
  pending:  { label: 'Pendiente',  bg: 'bg-amber-100 dark:bg-amber-950', text: 'text-amber-600 dark:text-amber-400' },
  verified: { label: 'Verificado', bg: 'bg-green-100 dark:bg-green-950', text: 'text-green-600 dark:text-green-400' },
  rejected: { label: 'Rechazado',  bg: 'bg-red-100 dark:bg-red-950',    text: 'text-red-500 dark:text-red-400'    },
};

// Fila de dato individual dentro del desplegable
const DetailRow = ({ label, value }) => (
  <div className="flex flex-col gap-0.5">
    <span className="text-[10px] font-semibold text-theme-muted uppercase tracking-wide">{label}</span>
    <span className="text-sm text-theme-text">{value || '—'}</span>
  </div>
);

const SellerRow = ({ seller, updating, onApprove, onReject, onPending }) => {
    ...
    <div>
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-semibold text-theme-text truncate">{seller.store_name}</p>
        ...
        <span className={`px-2.5 py-1 rounded-full text-xs font-medium shrink-0 ${cfg.bg} ${cfg.text}`}>
          {cfg.label}
        </span>
        ...
        <div className="flex gap-2 shrink-0" onClick={e => e.stopPropagation()}>
          ...
          {seller.status !== 'pending' && (
            <button onClick={onPending} disabled={isUpdating}
              className="px-3 py-1.5 rounded-lg border border-theme-border text-theme-secondary hover:bg-theme-muted text-xs font-semibold transition disabled:opacity-50">
      ...
      {expanded && (
        <div className="px-6 pb-6 bg-theme-subtle border-t border-theme-border">
          <div className="pt-4 grid grid-cols-2 gap-6">
            ...
            {/* Datos del usuario */}
            <div>
              <p className="text-xs font-bold text-theme-muted uppercase tracking-widest mb-3">👤 Datos del usuario</p>
              <div className="grid grid-cols-2 gap-3">
                <DetailRow label="Nombre"  value={`${seller.user?.name} ${seller.user?.last_name}`} />
                <DetailRow label="Email"   value={seller.user?.email} />
                <DetailRow label="Rol"     value={seller.user?.role} />
                <DetailRow label="Cuenta"  value={seller.user?.is_active ? 'Activa' : 'Inactiva'} />
              </div>
            </div>

            {/* Datos de la tienda */}
            <div>
              <p className="text-xs font-bold text-theme-muted uppercase tracking-widest mb-3">🏪 Datos de la tienda</p>
              <div className="grid grid-cols-2 gap-3">
            ...
            {/* Dirección de envío */}
            <div>
              <p className="text-xs font-bold text-theme-muted uppercase tracking-widest mb-3">📦 Dirección de envío</p>
              <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <p className="text-xs font-bold text-theme-muted uppercase tracking-widest mb-3">🚨 Incidencias</p>
              <div className="grid grid-cols-2 gap-3">
                <DetailRow label="En productos" value="— (próximamente)" />
                <DetailRow label="En pedidos"   value="— (próximamente)" />
                <DetailRow label="Solicitud"    value={seller.created_at?.split('T')[0]} />
              </div>
            </div>
            ...
            {seller.status === 'rejected' && seller.rejection_reason && (
              <div className="col-span-2">
                <p className="text-xs font-bold text-theme-muted uppercase tracking-widest mb-2">📋 Motivo del rechazo</p>
