...
const SellerRejected = ({ sellerProfile, onResubmit, onCancel }) => {
  ...
  return (
    <div className="space-y-4 max-w-3xl mx-auto px-4">
      ...
      <div className="bg-theme-bg border border-theme-border rounded-2xl p-8 shadow-sm text-center">
        <div className="text-5xl mb-4">❌</div>
        <h3 className="text-lg font-semibold text-theme-error mb-2">{t('seller.rejectedTitle')}</h3>
        <p className="text-theme-muted text-sm">{t('seller.rejectedDesc')}</p>
      </div>

      {/* Motivo del rechazo — solo si el admin lo especificó */}
      {sellerProfile.rejection_reason && (
        <div className="bg-theme-bg border border-theme-border rounded-2xl p-6 shadow-sm">
          <p className="text-xs font-bold text-theme-muted uppercase tracking-widest mb-2">
            📋 {t('seller.rejectionReason')}
          </p>
          <p className="text-sm text-theme-text bg-theme-error-bg border border-red-200 dark:border-red-900 rounded-xl px-4 py-3">
            {sellerProfile.rejection_reason}
          </p>
        </div>
      )}
      ...
      <div className="bg-theme-bg border border-theme-border rounded-2xl p-8 shadow-sm">
        <h2 className="text-lg font-semibold text-theme-text mb-1">✏️ {t('seller.editTitle')}</h2>
        <p className="text-theme-muted text-sm mb-8">{t('seller.editSubtitle')}</p>
        <SellerRegister
      ...
      <div className="bg-theme-bg border border-theme-border rounded-2xl p-6 shadow-sm text-center">
        <p className="text-sm text-theme-muted mb-3">{t('seller.cancelApplicationDesc')}</p>
        <button
