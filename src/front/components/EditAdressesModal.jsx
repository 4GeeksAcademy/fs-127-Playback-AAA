...
const EditAddressModal = ({ address, onClose, onSaved }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!selectedCommunity) return setError("Selecciona una comunidad autónoma.");
    ...
    const [data, err] = await addressService.updateAddress(token, address.id, form);
    setLoading(false);
  ...
  const inputClass = "w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-violet-400";
  const selectClass = `${inputClass} bg-white`;
  const labelClass = "block text-xs text-gray-500 mb-1";
  const sectionTitle = "text-xs font-semibold tracking-widest text-gray-400 uppercase mb-3";
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 overflow-hidden">
        ...
        <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
          <h2 className="text-base font-semibold text-gray-800">Editar dirección</h2>
          ...
          <div>
            ...
            <div className="space-y-3">
              <div>
                ...
                <input name="full_name" value={form.full_name || ""} onChange={handleChange} className={inputClass} required />
              </div>
              <div>
          ...
          <div>
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <select value={selectedCommunity} onChange={handleCommunityChange} className={selectClass} required>
                  <option value="">{communities.length === 0 ? "Cargando..." : "Selecciona comunidad"}</option>
              ...
              <div>
                ...
                {selectedCommunity ? (
                  <select value={selectedProvince} onChange={handleProvinceChange} disabled={loadingProvinces} className={selectClass} required>
                  ...
                  <input name="province" value={form.province || ""} onChange={handleChange} className={inputClass} readOnly />
                )}
              </div>
              ...
              <div>
                ...
                {selectedProvince ? (
                  <select value={selectedMunicipality} onChange={handleMunicipalityChange} disabled={loadingMunicipalities} className={selectClass} required>
                    <option value="">{loadingMunicipalities ? "Cargando..." : "Selecciona municipio"}</option>
                    {municipalities.map((m) => <option key={m.CMUN} value={m.CMUN}>{m.ALTERNATIVO_DMUN50 || m.DMUN50}</option>)}
                  </select>
                ) : (
                  <input name="municipality" value={form.municipality || ""} onChange={handleChange} className={inputClass} readOnly />
                )}
              </div>
              ...
              <div>
                ...
                <input name="address" value={form.address || ""} onChange={handleChange} placeholder="Calle, número, piso..." className={inputClass} required />
              </div>
        ...
        <div className="px-6 py-4 border-t border-gray-100 flex gap-3 justify-end">
          ...
          <button type="button" disabled={loading} onClick={handleSubmit} className="px-4 py-2 text-sm text-white bg-violet-600 rounded-lg hover:bg-violet-700 disabled:opacity-60 transition">
