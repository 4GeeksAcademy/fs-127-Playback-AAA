...
export const TopRatedSubcategories = () => {
    ...
    if (loading) return (
        <section>
            <h2 className="text-lg font-semibold my-4 text-theme-text">Categorías Destacadas</h2>
        ...
        <section>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                {subcategories.map((sub) => (
                    ...
                    <div
                        key={sub.id}
                        onClick={() => navigate(`/products?category=${sub.category_slug}&subcategory=${sub.slug}`)}
