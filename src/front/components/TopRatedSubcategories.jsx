...
import { getTopRatedSubcategoriesService, getCategoriesService } from "../services/categoryService";
      ...
      if (data && data.length > 0) {
        ...
        getCategoriesService().then(([catData]) => {
          if (catData?.length) {
            ...
            const shuffled = [...allSubs].sort(() => Math.random() - 0.5).slice(0, 5);
        ...
        // Al hacer click navega a productos filtrando por categoría y subcategoría
          <div
            ...
            className={`cursor-pointer group hover:shadow-md transition-all duration-200 bg-main border border-main
              ${subcategories.length % 2 !== 0 && index === subcategories.length - 1
                ? "sm:col-span-2 lg:col-span-1"
                : ""
              }`}
          >
            <img
              src={sub.image_url || "https://placehold.co/400x300?text=Sin+imagen"}
              alt={getName(sub)}
