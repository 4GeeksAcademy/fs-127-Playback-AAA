...
const ProfileIncidents = () => {
  ...
  if (loading) return (
    <p className="text-center text-sm text-muted mt-10 animate-pulse">Cargando incidencias…</p>
  );
  ...
  return (
    <div className="space-y-3">
      {incidents.map(incident => {
        const cfg  = STATUS_CONFIG[incident.status] || { label: incident.status, bg: "#F1EFE8", color: "#5F5E5A" };
          ...
          <div key={incident.id} className="bg-main rounded-xl border border-main overflow-hidden">
            <div className="px-4 py-4">
              ...
              <div className="flex items-start justify-between gap-3">
                ...
                <span style={{ background: cfg.bg, color: cfg.color, fontSize: "11px", padding: "2px 7px", borderRadius: "20px", fontWeight: "500", flexShrink: 0 }}>
                  {cfg.label}
                </span>
              </div>

              <div className="flex items-center justify-between mt-3" style={{ fontSize: "11px", color: "var(--color-muted)" }}>
                <span>Pedido <span style={{ fontFamily: "monospace", fontWeight: "600", color: "var(--color-text-primary)" }}>#{incident.order_id}</span></span>
                <span>{date}</span>
              </div>
