...
export const SearchBar = ({ placeholder = "", className = "" }) => {
  ...
  const handleChange = useThrottledCallback((value) => {
    ...
    if (!value) {
      ...
      setLoading(true); // feedback inmediato al usuario
      fetchResults(value);
    }
  }, 50);
  ...
  return (
    <div ref={containerRef} className={`relative w-full ${className}`}>
      ...
      <div className="relative">
        <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 pointer-events-none w-5 h-5 text-theme-muted" />
        ...
        <input
          ...
          className="w-full h-10 pl-10 pr-9 rounded-full border border-theme-border bg-theme-input text-theme-text placeholder-theme-faint text-sm focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-transparent transition"
        />
      ...
      {open && (
        <div className="absolute top-full mt-2 left-0 right-0 bg-theme-bg border border-theme-border rounded-2xl shadow-2xl z-50 overflow-hidden max-h-80 overflow-y-auto">
            ...
            <>
              {results.map((product) => (
                <button
                  ...
                  className="w-full flex items-center gap-3 px-4 py-2.5 hover:bg-theme-muted transition text-left"
                >
                  {/* Miniatura */}
                  {product.image_url ? (
                    <img
                      ...
                      alt={product.name}
                      className="w-10 h-10 rounded-xl object-cover flex-shrink-0 border border-theme-border"
                    />
                  ) : (
                    <div className="w-10 h-10 rounded-xl bg-theme-muted flex-shrink-0" />
                  )}
              ...
              >
                Ver todos los resultados para "{inputValue}" →
              </button>
            </>
          )}
        </div>
