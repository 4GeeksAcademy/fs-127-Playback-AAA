...
def create_product():
    ...
    if request.content_type and "multipart/form-data" in request.content_type:
        ...
        # Imágenes adicionales: múltiples ficheros con key "other_images"
        ...
        imagen          = None
        # En JSON puede llegar como lista de URLs ya subidas
    ...
    # ── Imágenes adicionales (solo si vienen como ficheros)
    other_image_url = other_images  # si es JSON ya es lista de URLs
...
def update_product(id):
    ...
    if request.content_type and "multipart/form-data" in request.content_type:
        ...
        imagen          = None
        other_images    = body.get("other_image_url")  # lista de URLs o None
    ...
    try:
        ...
        if other_images:
            if hasattr(other_images[0], "read"):
                # Son ficheros → subir a Cloudinary
                product.other_image_url = [
                    cloudinary.uploader.upload(img, folder="productos")["secure_url"]
                    for img in other_images
                ]
            else:
                # Son URLs directamente (JSON)
